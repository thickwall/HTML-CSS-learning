...
function load(){
	...
	for(i = 0;i < input.length;i++)
		if (input[i].type != "submit"){
			EventUtil.addHandler(input[i], "blur", function(){validate(this)});
			EventUtil.addHandler(input[i], "focus", function(){valid(this)});
		}

	Fill_P_C(document.getElementById("Province"), MAP);
	EventUtil.addHandler(document.getElementById("Province"), "change", ChangePro);
	EventUtil.addHandler(document.getElementById("City"), "change", ChangeCit);
...
function valid(t){
	if(t.id == "Pwd")
		valid(document.getElementById("Pwd-confirm"));	//清除“密码不一致”的提示
	if(t.id == "Sex-m")
		valid(document.getElementById("Sex-f"));
	if(!t.nextElementSibling || t.nextElementSibling.tagName != "SPAN"){
		return;
	}
	else{
		var info = t.nextElementSibling;
	}
	info.style.display = "none";
}
function validate(t){
	if(t.id == "Uname")
		return VerifyUname(t);
	else if (t.id == "Pwd" || t.id == "Pwd-confirm"){
			//检验密码格式
		if (t.id == "Pwd" && document.getElementById("Pwd-confirm").value != '')
			return VerifyPWD(t) && ConfirmPWD();	//检验密码是否一致
		else if (t.id == "Pwd-confirm")
		    return VerifyPWD(t) && ConfirmPWD();
		else {
		    return VerifyPWD(t);
		}
	}
		...
		return VerifyPhone(t);
	}
}
function VerifyUname(t){
	if (t.value == ''){
			setInfo(t, "info-warning", "字段为空");
			return false;
	}
	else{
	//校验字符是否合法
		var patrn=/^[a-zA-Z]{1}([a-zA-Z0-9]){4,19}$/;
		if (!patrn.exec(t.value)) {
		    setInfo(t, "info-error", "5-20位 字母开头 只含数字、字母");
		    return false;
		}
	}
	return true;
}
function VerifyPWD(t){
	if (t.value == ''){
			setInfo(t, "info-warning", "字段为空");
			return false;
	}
		...
		if (!patrn.exec(t.value)) {
		    setInfo(t, "info-error", "6-20位 数字、字母、下划线");
		    return false;
		}
	}
	return true;	
...
function ConfirmPWD(){
	...
	var con = document.getElementById("Pwd-confirm");
	if (pwd.value != con.value){
		setInfo(con, "info-error", "密码不一致");
		return false;		
	}
	return true;
}
function VerifySex(){
	sex_selected = (document.querySelector("#Sex-m").checked || document.querySelector("#Sex-f").checked)
	if(!sex_selected){
	    setInfo(document.querySelector("#Sex-f"), "info-warning", "请选择一个性别");
	    return false;
	}
	return true;
}
function VerifyMail(t){
	if (t.value == ''){
			setInfo(t, "info-warning", "字段为空");
			return false;
	}
	else if (!t.validity.valid){
			setInfo(t, "info-error", "格式不正确");
			return false;
		//校验字符是否合法
	}	
	return true;
...
function ChangePro(){
	var Pro = this, City = document.getElementById("City"), Town = document.getElementById("Town");
	var Items = Pro.options, selected = Items[this.selectedIndex].text;
	if(selected == "北京市" || selected == "上海市" || selected == "天津市"){
		Town.style.display = "none";
		Fill_T(City, MAP[selected]["市辖区"]);
	}
	else if (selected == "澳门特别行政区" || selected == "香港特别行政区" || selected == "台湾省"){
		Clear_Selector(City);
		Clear_Selector(Town);
		City.disabled = true;
		Town.disabled = true;
		...
		Town.disabled = false;
		City.disabled = false;
		Fill_P_C(City, MAP[selected]);	
	}
}
function ChangeCit(){
	var Pro = document.getElementById("Province"), City = this, Town = document.getElementById("Town");
	var Items = City.options, selected = Items[this.selectedIndex].text;
	var t = MAP[Pro.options[Pro.selectedIndex].text][selected];
	if (t && t.length) {
	    Town.style.display = "inline-block";
		Clear_Selector(Town);
		Fill_T(Town, t);
...
function setInfo(t, tclass, text){
	var p = t.parentNode;
	var b_insert = false;
	if(!t.nextElementSibling || t.nextElementSibling.tagName != "SPAN"){
	...
	info.setAttribute("class", tclass);
	info.innerText = text;
	...
	if(b_insert){
		if (p.lastChild == t) {
			...
			p.insertBefore(info, t.nextElementSibling);
...
function setSpacing(){
 	//调整 用户名这三个字的子间距使两边对齐
 	var word = document.getElementById("form-basic").children[0];
	word.children[0].style.display="inline-block";
	word.children[2].style.display="inline-block";
	var std_width = word.getElementsByTagName("span")[1].clientWidth;
	var offset = (std_width - word.children[0].clientWidth) / 2;
	word.children[0].style.wordSpacing= offset + "px";
	if(word.children[0].clientWidth < std_width)
		 word.children[0].style.wordSpacing= (offset + 1) + "px";	
}
...
function AJAXTest() {
    ...
    xml.open("post", "http://test.api.snsports.cn/api_passport/login.do", true);
    // xml.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xml.send("username=15001765110&password=123456");
